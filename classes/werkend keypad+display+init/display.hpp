...
private:
    rtos::flag flagDisplayRead;
    rtos::pool<const char*> displayPoolRead;
    DisplayClass & display;
    void main(){
        for(;;){ 
            auto evt = wait(flagDisplayRead);
            if(evt == flagDisplayRead){
                hwlib::cout<<"Clear display\n";
                display.clearDisplay();
                hwlib::wait_ms(1000);
                display.setDisplay(displayPoolRead.read(), 1);
                hwlib::wait_ms(1000);
...
public:
    DisplayTask(DisplayClass & displayClass): task("displayTask"), flagDisplayRead(this, "flagDisplayRead"), displayPoolRead("displayPoolRead"), display(displayClass){}
    void writeDisplay(const char* text){ displayPoolRead.write(text); flagDisplayRead.set(); }
};
